...
public class ChessGame {
    ...
    public static boolean isGameEnded = false,iwon=false,oppwon=false;
    static JFrame frame;
    static JPanel pn;
    ...
    public static void main(String[] args) throws IOException, InterruptedException, InvalidName, NotFound, CannotProceed, org.omg.CosNaming.NamingContextPackage.InvalidName {
        Update.main(new String[0],Client.id);
    ...
    if (selectedPiece != null) {
        ...
        if (moved) {
            ...
            sendData sd=new sendData(new data(exp[0], exp[1], e.getX(), e.getY(), Client.id,false));
        ...
        if (Client.isWhitePlayer==false){
            sendData sd=new sendData(new data(-1,-1,-1,-1,Client.id,false));
            sd.start();
